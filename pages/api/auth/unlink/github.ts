...
apiRoute.post(async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const { data: userData } = await supabase.from("users").select("github").eq("id", cookie.user.id);
    if (!userData[0].github.id) {
        return res.status(500).json({
            error: "There is no Github account to unlink.",
        });
    }

    const { error } = await supabase.from("users").update({ github: {} }).eq("id", cookie.user.id);
    if (error) {
        return res.status(500).json({
