...
apiRoute.put(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    await supabase.from("users").update({ avatar: `${process.env.NEXT_PUBLIC_URL ? process.env.NEXT_PUBLIC_URL : process.env.VERCEL_URL}/api/avatars/default.jpg` }).eq("id", user.id);
    await supabase.auth.update({
      data: {
        avatar: `${process.env.NEXT_PUBLIC_URL ? process.env.NEXT_PUBLIC_URL : process.env.VERCEL_URL}/api/avatars/default.jpg`,
      }
    });
