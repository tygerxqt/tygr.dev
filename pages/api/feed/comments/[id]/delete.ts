...
apiRoute.delete(async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const { data: commentUser, error: fetchErr } = await supabase.from("feed_comments").select("user").eq("id", req.query.id);
    if (fetchErr) {
    ...
    if (admin || cookie.user.id === commentUser[0].user_id) {
        const { error: delError } = await supabase.from("feed_comments").delete().eq("id", req.query.id);
        if (delError) {
