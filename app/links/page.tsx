	...
	return (
		<>
			<div className="flex flex-col gap-2 pt-2 w-full max-w-[800px]">
				<div className="pb-4">
					<h2 className="text-2xl font-bold sm:text-3xl font-display">Links</h2>
					<p className="text-sm">
						A bunch of useful links ranging from all my social media, resources
						that are helpful, to even my wishlist.
					</p>
				</div>
				{linksByType &&
					Object.keys(linksByType).map((type, i) => (
						<div key={i} className="flex flex-col gap-2 pb-2">
							<h3 className="text-lg font-bold capitalize underline-offset-2">
								{type}s
							</h3>
							{linksByType[type].map((link: Link, i: number) => (
								...
								>
									<p className="flex flex-row items-center gap-2 font-semibold">
										<Image
											src={`https://t2.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${link.link}&size=16`}
											width={16}
											height={16}
											alt=""
											aria-label={`Favicon for ${link.name}`}
										/>{" "}
